...
@Component({
  ...
  styles: `
    ...
    .post-card-book-cover-box {
        ...
        &::before {
            @apply content-space block pt-[120%];
        }
    }
    .post-card-chat{
    ...
    <div class="post-card">
      <div class="post-card-header">
        <div class="post-card-main">
          <img class="post-card-user-pic" src="https://pictures.abebooks.com/isbn/9780745647791-us.jpg">
          <div class="post-card-titling">
            <a class="post-card-user-name" [routerLink]="['/', 'books', 'inventory', post.userId]">{{ post.user.firstName }} {{ post.user.lastName }}</a>
            <span class="post-card-date-time">{{ post.postedOn | date:'MMM d, Y HH:mm' }}</span>
          </div>
        </div>
      </div>
      <div class="post-card-body">
        <div class="post-card-book">
          <a class="post-card-book-cover-box ratio-2/3" [routerLink]="['/', 'books', 'markets', post.book.isbn13]">
            <img class="post-card-book-cover" [src]="post.book.thumbnail">
          </a>
          <a class="post-card-book-title" [routerLink]="['/', 'books', 'markets', post.book.isbn13]" >{{ post.book.title }} {{ post.book.subtitle }}</a>
