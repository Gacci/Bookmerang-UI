...
export class PasswordStrengthComponent {
  ...
  set password(value: string | null) {
    ...
    this.highlight = this.result.percent > 0 && this.result.percent <= 20 ? 'bg-red-300'
      : this.result.percent > 20 && this.result.percent <= 40 ? 'bg-orange-300'
      : this.result.percent > 40 && this.result.percent <= 60 ? 'bg-yellow-300'
      : this.result.percent > 60 && this.result.percent <= 80 ? 'bg-green-300'
      : this.result.percent > 80 ? 'bg-green-500' : '';
